### v0.9.4-rc1
- Unifies the `pbs`, `signer`, and `cli` binaries into one: `commit-boost`. This change changes the CLI, notably the `cli --init` command is now invoked as `commit-boost cli init`.
