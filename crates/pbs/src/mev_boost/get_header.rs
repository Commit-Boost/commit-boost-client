...
use cb_common::{
    ...
    utils::{
        EncodingType, OUTBOUND_ACCEPT, get_user_agent_with_version, ms_into_slot,
...
async fn send_get_header_impl(
    ...
    // Per the builder spec, 200 carries a bid payload and 204 means no bid
    // is available. Any other status is an error. We check before reading
    // the body so that response headers are still accessible for
    // Content-Type and Eth-Consensus-Version parsing on the 200 path.
