...
use cb_common::pbs::{
    ...
    SUBMIT_BUILDER_PREFERENCES_PATH, SUBMIT_SIGNED_BEACON_BLOCK_PATH,
};
...
use super::{
    ...
    handle_submit_signed_beacon_block, reload::handle_reload,
};
use crate::{
    MAX_SIZE_REGISTER_VALIDATOR_REQUEST, MAX_SIZE_SUBMIT_BLOCK_RESPONSE,
    MAX_SIZE_SUBMIT_SIGNED_BEACON_BLOCK,
    api::BuilderApi,
...
pub fn create_app_router<S: BuilderApiState, A: BuilderApi<S>>(state: PbsStateGuard<S>) -> Router {
    ...
    let v1_builder_routes = Router::new()
        ...
        .route(
            ...
            post(handle_submit_signed_beacon_block::<S>)
                .route_layer(DefaultBodyLimit::max(MAX_SIZE_SUBMIT_SIGNED_BEACON_BLOCK)),
        );
...
#[tracing::instrument(
    ...
    fields(
        ...
        parent_root = tracing::field::Empty,
