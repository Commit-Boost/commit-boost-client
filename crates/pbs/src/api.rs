...
#[async_trait]
pub trait BuilderApi<U: Send + Sync + 'static, S: BuilderApiState>: 'static {
    /// Use to extend the BuilderApi
    fn extra_routes() -> Option<Router<PbsState<U, S>>> {
        None
    }
    ...
    async fn get_status(req_headers: HeaderMap, state: PbsState<U, S>) -> eyre::Result<()> {
