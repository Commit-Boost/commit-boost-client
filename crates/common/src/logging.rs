...
use tracing_subscriber::{
    ...
    prelude::*,
    registry::LookupSpan,
    EnvFilter, Layer as SubscriberLayer, Registry,
};

use crate::config::{load_optional_env_var, LogDest, LogFormat, LogsSettings};
    ...
    N: for<'a> fmt::FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        writeln!(writer, "{}", serde_json::to_string(&output).map_err(|_| std::fmt::Error)?)
    }
}
    ...
    match settings.destination {
        ...
        LogDest::Stdout => {
            ...
            let layer: Box<dyn SubscriberLayer<Registry> + Send + Sync> = match settings.format {
                LogFormat::Json => {
                    Box::new(fmt::layer().event_format(JsonFormatter).with_writer(writer))
                }
                LogFormat::Raw => {
                    Box::new(fmt::layer().event_format(RawFormatter).with_writer(writer))
                }
        ...
        LogDest::File => {
            ...
            let layer: Box<dyn SubscriberLayer<Registry> + Send + Sync> = match settings.format {
                LogFormat::Json => {
                    Box::new(fmt::layer().event_format(JsonFormatter).with_writer(file_writer))
                }
                LogFormat::Raw => {
                    Box::new(fmt::layer().event_format(RawFormatter).with_writer(file_writer))
                }
        ...
        LogDest::Both => {
            ...
            let stdout_layer: Box<dyn SubscriberLayer<Registry> + Send + Sync> =
                match settings.format {
                    LogFormat::Json => Box::new(
                        fmt::layer().event_format(JsonFormatter).with_writer(stdout_writer),
                    ),
                    LogFormat::Raw => {
                        Box::new(fmt::layer().event_format(RawFormatter).with_writer(stdout_writer))
                    }
...
fn format_crates_filter(default_level: &str, crates_level: &str) -> Result<EnvFilter> {
    let s = format!(
...
//                     min_bid_wei: alloy::primitives::U256::from(10000),
//                     relay_monitors: vec![],
