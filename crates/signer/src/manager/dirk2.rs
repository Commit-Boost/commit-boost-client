...
use alloy::{
    hex, primitives::FixedBytes, rpc::types::beacon::constants::BLS_SIGNATURE_BYTES_LEN,
...
pub struct DirkManager {
    chain: Chain,
    certs: CertConfig,
    channels2: HashMap<Url, Channel>,   // endpoint -> channel
...
impl DirkManager {
    pub async fn new_from_config(chain: Chain, config: DirkConfig) -> eyre::Result<Self> {
        ...
        let mut channels = HashMap::with_capacity(tls_configs.len());
        ...
        for host in config.hosts.clone() {
            ...
            for account_name in host.accounts.clone() {
                ...
                for dirk_account in dirk_accounts.iter().filter(|a| {
                    ...
                    let key_name = dirk_account
                        .name
                        .split_once("/")
                        .map(|(_wallet, name)| name)
                        .unwrap_or_default();
        ...
        let certs = CertConfig {
            ...
            ca_cert: config.cert_auth.clone(),
        };
        for host in config.hosts {
            let server = ServerInfo { endpoint: host.url.clone(), server_name: host.server_name };
            let channel = match connect(server, certs.clone()).await {
                Ok(channel) => channel,
                Err(e) => {
                    warn!("Couldn't connect to Dirk server {}: {e}", host.url);
                    continue;
                }
            };

            channels2.insert(host.url.clone(), channel.clone());

            for account in host.accounts {
                let Some((wallet, name)) = account.split_once('/') else {
                    warn!("Invalid account name: {account}");
                    continue;
                };

                let res = ListerClient::new(channel.clone())
                    .list_accounts(ListAccountsRequest { paths: vec![account.clone()] })
                    .await
                    .unwrap();

                if let Some(distributed_account) = res.get_ref().distributed_accounts.get(0) {
                    let pubkey = distributed_account.composite_public_key.clone();
                    let acc = Account {
                        wallet: wallet.to_string(),
                        name: name.to_string(),
                        public_key: Some(BlsPublicKey::try_from(pubkey.as_slice()).unwrap()),
                        hosts: distributed_account
                            .participants
                            .iter()
                            .map(|participant| HostInfo {
                                url: format!("{}/{}", participant.name, participant.port),
                                participant_id: participant.id,
                            })
                            .collect(),
                        wallet_type: WalletType::Distributed,
                        signing_threshold: distributed_account.signing_threshold,
                        is_proxy: is_proxy_key_name(&account),
                    };
                    accounts.insert(account, acc);
                } else if let Some(simple_account) = res.get_ref().accounts.get(0) {
                    let pubkey = simple_account.public_key.clone();
                    let acc = Account {
                        wallet: wallet.to_string(),
                        name: name.to_string(),
                        public_key: Some(BlsPublicKey::try_from(pubkey.as_slice()).unwrap()),
                        hosts: vec![HostInfo { url: host.url.to_string(), participant_id: 1 }],
                        wallet_type: WalletType::Simple,
                        signing_threshold: 1,
                        is_proxy: is_proxy_key_name(&account),
                    };
        ...
        let mut manager = Self {
            chain,
            certs: CertConfig { client_cert: config.client_cert, ca_cert: config.cert_auth },
            channels2,
            channels,
            accounts,
...
async fn connect(server: ServerInfo, certs: CertConfig) -> eyre::Result<Channel> {
    let mut tls_config = ClientTlsConfig::new().identity(certs.client_cert);
    ...
    Channel::from_shared(server.endpoint.to_string())
