...
#[tokio::test]
async fn test_get_header_deneb() -> Result<()> {
    setup_test_env();
    ...
    tokio::spawn(start_mock_relay_service::<DenebSpec>(mock_state.clone(), relay_port));
    ...
    let res = serde_json::from_slice::<GetHeaderResponse<DenebSpec>>(&res.bytes().await?)?;
...
#[tokio::test]
async fn test_get_header_electra() -> Result<()> {
    setup_test_env();
    let signer = random_secret();
    let pubkey: BlsPublicKey = blst_pubkey_to_alloy(&signer.sk_to_pk()).into();
    ...
    let mock_state = Arc::new(MockRelayState::new(chain, signer));
    let mock_relay = generate_mock_relay(relay_port, *pubkey)?;
    tokio::spawn(start_mock_relay_service::<ElectraSpec>(mock_state.clone(), relay_port));
    ...
    let res = serde_json::from_slice::<GetHeaderResponse<ElectraSpec>>(&res.bytes().await?)?;
...
#[tokio::test]
async fn test_get_header_returns_204_if_relay_down_deneb() -> Result<()> {
    setup_test_env();
    let signer = random_secret();
    let pubkey: BlsPublicKey = blst_pubkey_to_alloy(&signer.sk_to_pk()).into();
...
#[tokio::test]
async fn test_get_header_returns_204_if_relay_down_electra() -> Result<()> {
    setup_test_env();
    let signer = random_secret();
    let pubkey: BlsPublicKey = blst_pubkey_to_alloy(&signer.sk_to_pk()).into();
...
#[tokio::test]
async fn test_get_header_returns_400_if_request_is_invalid_deneb() -> Result<()> {
    setup_test_env();
    let signer = random_secret();
    let pubkey: BlsPublicKey = blst_pubkey_to_alloy(&signer.sk_to_pk()).into();
...
#[tokio::test]
async fn test_get_header_returns_400_if_request_is_invalid_electra() -> Result<()> {
    setup_test_env();
    let signer = random_secret();
    let pubkey: BlsPublicKey = blst_pubkey_to_alloy(&signer.sk_to_pk()).into();
