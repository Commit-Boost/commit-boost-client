...
async fn test_get_status() -> Result<()> {
    ...
    let relay_0_port = relay_0_listener.local_addr()?.port();
    let relay_1_port = relay_1_listener.local_addr()?.port();
...
async fn test_get_status_returns_502_if_relay_down() -> Result<()> {
    ...
    // Release the relay's port so nothing listens on it: the relay is down
    drop(relay_listener);
