...
#[tokio::test]
async fn test_mux_deneb() -> Result<()> {
    setup_test_env();
    ...
    tokio::spawn(start_mock_relay_service::<DenebSpec>(mock_state.clone(), pbs_port + 2));
    tokio::spawn(start_mock_relay_service::<DenebSpec>(mock_state.clone(), pbs_port + 3));
    ...
    assert_eq!(mock_validator.do_submit_block::<DenebSpec>(None).await?.status(), StatusCode::OK);
...
#[tokio::test]
async fn test_mux_electra() -> Result<()> {
    setup_test_env();
    let signer = random_secret();
    let pubkey: BlsPublicKey = blst_pubkey_to_alloy(&signer.sk_to_pk()).into();
    ...
    tokio::spawn(start_mock_relay_service::<ElectraSpec>(mock_state.clone(), pbs_port + 2));
    tokio::spawn(start_mock_relay_service::<ElectraSpec>(mock_state.clone(), pbs_port + 3));
    ...
    // Submit block requests should go to all relays
    info!("Sending submit block");
    assert_eq!(mock_validator.do_submit_block::<ElectraSpec>(None).await?.status(), StatusCode::OK);
    assert_eq!(mock_state.received_submit_block(), 3); // default + 2 mux relays were used
