...
async fn test_signer_sign_request_good() -> Result<()> {
    ...
    let pubkey = FixedBytes::new(hex!("883827193f7627cd04e621e1e8d56498362a52b2a30c9a1c72036eb935c4278dee23d38a24d2f7dda62689886f0c39f4"));
    let object_root = b256!("0x0123456789012345678901234567890123456789012345678901234567890123");

    // Send a signing request
    let request = SignRequest::Consensus(SignConsensusRequest { pubkey, object_root });
    ...
    info!("Server returned expected success code {} for signing request", response.status());
