...
pub struct MockRelayState {
    ...
    /// When true, the bid 200 claims `capella` instead of gloas (header on
    /// SSZ, body `version` on JSON). Drives the PBS gloas-only response check.
    epbs_wrong_fork: bool,
}
...
impl MockRelayState {
    ...
    /// Label the bid 200 `capella` instead of gloas, to exercise the PBS
    /// gloas-only response check (the bid must be dropped, not forwarded).
    pub fn with_epbs_wrong_fork(self) -> Self {
        Self { epbs_wrong_fork: true, ..self }
    }
}
