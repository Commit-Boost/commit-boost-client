...
Administrative endpoints (`/reload`, `/revoke_jwt`) authenticate with a **separate JWT** signed with the `CB_SIGNER_ADMIN_JWT` secret (env var). Same HS256 algorithm, includes `admin: true` in its claims.

### Rate limiting

The signer rate-limits by IP address. Default: **3 failed authentications within 5 minutes** locks a client out. Configurable via `[signer]` in `cb-config.toml`:
...
If running behind a reverse proxy, configure the [reverse proxy header setup](../get_started/configuration.md#rate-limit) so the correct client IP is extracted.

---

## API Quickstart

Below is a walkthrough of the full Signer API flow using the Rust SDK. The `SignerClient` (returned by `load_commit_module_config`) handles JWT creation, payload hashing, and token refresh automatically — you never craft JWTs by hand.
...
| HTTP Status | Meaning |
|-------------|---------|
| `400` | Malformed request body, invalid pubkey format, missing signing ID, or operation not supported by current backend (e.g. ECDSA proxy with Dirk). |
| `401` | Missing or invalid JWT. Token may be expired, signed with wrong secret, or missing required claims. |
