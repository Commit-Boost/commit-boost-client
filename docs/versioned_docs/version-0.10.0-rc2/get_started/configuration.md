...
Proxy keys can be used to sign transactions with a different key than the one used to sign the block. Proxy keys are generated by the Signer Service and authorized by the validator key. Each module have their own proxy keys, that can be BLS or ECDSA.
...
To learn more about developing modules, check out [here](../developing/custom-modules).
